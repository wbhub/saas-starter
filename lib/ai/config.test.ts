...
describe("lib/ai/config env parsing", () => {
  ...
  it("parses by_plan rules and default monthly budget values", async () => {
    ...
    vi.stubEnv(
      "AI_PLAN_RULES_JSON",
      JSON.stringify({
        free: { enabled: true, model: "gpt-4.1-mini", monthlyBudget: 1000.2 },
        starter: { enabled: true, model: "gpt-4.1", monthlyBudget: 2000 },
        growth: { enabled: false, model: "gpt-4.1", monthlyBudget: -4 },
        pro: { enabled: true, model: null, monthlyBudget: 9999 },
      }),
    );
